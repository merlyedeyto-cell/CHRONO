...
$id = intval($_GET['id']);
if ($userCol) {
    $stmt = $conn->prepare("SELECT image_path FROM memories WHERE id = ? AND `$userCol` = ?");
    $stmt->bind_param("ii", $id, $userId);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $result = $conn->query("SELECT image_path FROM memories WHERE id=$id");
}
...
if ($userCol) {
    $sql = "UPDATE memories SET image_path = NULL WHERE id = ? AND `$userCol` = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $id, $userId);
} else {
    $sql = "UPDATE memories SET image_path = NULL WHERE id = ?";
    $stmt = $conn->prepare($sql);
